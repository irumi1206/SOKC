#include <iostream>
#include "/home/ubuntu/GameProject/Project/SOKC/gameController.cpp"
...
TEST(HEADER_30,moving_check){
    ...
    controller.control("{\"Header\":30,\"id\":"+std::to_string(id1)+",\"x\":3.140000,\"y\":6.20000}");
    controller.control("{\"Header\":30,\"id\":"+std::to_string(id2)+",\"x\":3.530000,\"y\":0.700000}");
    controller.control("{\"Header\":30,\"id\":"+std::to_string(id3)+",\"x\":6.310000,\"y\":10.300000}");
...
TEST(HEADER_30,get_position_check){
    ...
    Json::Value toAll;
    toAll["Header"]=30;
    toAll["playerPositions"]="[(\"id\":"+to_string(id1)+",\"x\":3.140000,\"y\":6.200000),(\"id\":"+to_string(id2)+",\"x\":3.530000,\"y\":0.700000),(\"id\":"+to_string(id3)+",\"x\":6.310000,\"y\":10.300000)]";
    out["toAll"]=toAll;
...
TEST(HEADER_31,kill_check){
    ...
    out["toOne"]=toOne;
    out["toAll"]=toAll;
    EXPECT_EQ(controller.control("{\"Header\":31,\"id\":"+to_string(id1)+",\"victim\":"+to_string(id2)+"}"),out);
...
TEST(HEADER_32,mission_check){
    ...
    controller.control("{\"Header\":32,\"id\":"+to_string(id1)+",\"missionId\":1}");
    ...
    controller.control("{\"Header\":32,\"id\":"+to_string(id3)+",\"missionId\":"+to_string(missions[0])+"}");
    EXPECT_EQ(controller.game.findPlayer(id3).countMission(),2);
    EXPECT_EQ(controller.game.findPlayer(id3).getMissionScore(),1);
    controller.control("{\"Header\":32,\"id\":"+to_string(id3)+",\"missionId\":"+to_string(missions[1])+"}");
    EXPECT_EQ(controller.game.findPlayer(id3).countMission(),1);
    EXPECT_EQ(controller.game.findPlayer(id3).getMissionScore(),2);
    controller.control("{\"Header\":32,\"id\":"+to_string(id3)+",\"missionId\":"+to_string(missions[2])+"}");
    EXPECT_EQ(controller.game.findPlayer(id3).countMission(),0);

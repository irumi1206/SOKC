...
void TcpService::looping(){
    std::cout<< "looping started";
    
    while(1){
        std::string positionJson = controller.positions().toStyledString();
        usleep(1000000);
        std::cout<<"about to send, Storage size : "<<udpStorage.size()<<"\n";
        for(int i=0;i<udpStorage.size();++i){
            char charOut[1024]={0};
            strcpy(charOut,positionJson.c_str());
            // const char* ou="position";
            struct sockaddr_in r=udpStorage[i];
            //std::cout<<"ip address : "<<inet_ntoa(r.sin_addr)<<" port : "<<ntohs(r.sin_port)<<"\n";
            sendto(udpSocket, charOut, strlen(charOut), 0,(const struct sockaddr *) &r, sizeof(r));
        }
    }
